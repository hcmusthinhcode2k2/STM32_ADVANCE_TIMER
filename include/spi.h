...
typedef struct
{
  ...
  BUNION(I2SCFGR, unsigned long,                 //              0x0000'0000     SPI_I2S configuration register
    ...
    _reserved1                          , 20);
  BUNION(I2SPR, unsigned long,                   //              0x0000'0010     SPI_I2S prescaler register
...
typedef struct
{
  RSTRUCT(CR1, unsigned long,                    //              0x0000'0000     SPI control register 1 (not used in I2S mode)
    ...
    _reserved                           [16]);
  RSTRUCT(CR2, unsigned long,                    //              0x0000'0000     SPI control register 2
    ...
    _reserved1                          [24]);
  RSTRUCT(SR, unsigned long,                     //              0x0000'0002     SPI status register
  ...
  unsigned long const RXCRCR;                    //              0x0000'0000     SPI RX CRC register (SPI_RXCRCR) (not used in I2S mode)
  unsigned long const TXCRCR;                    //              0x0000'0000     SPI TX CRC register (SPI_TXCRCR) (not used in I2S mode)
  RSTRUCT(I2SCFGR, unsigned long,                //              0x0000'0000     SPI_I2S configuration register
    ...
    _reserved1                          [20]);
  RSTRUCT(I2SPR, unsigned long,                  //              0x0000'0010     SPI_I2S prescaler register
    I2SDIV                              [8],    //0:7           0               I2S Linear prescaler
...
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
void SPI_Write(volatile SPI_TypeDef* SPI, unsigned long Data);                   //Data: 16 or 8 bit
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
unsigned long SPI_Read(volatile SPI_TypeDef* SPI);                               //return: 16 or 8 bit
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
unsigned long SPI_WriteRead(volatile SPI_TypeDef* SPI, unsigned long Data);       //Data: 16 or 8 bit, return: 16 or 8 bit
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
void SPI_Writes(volatile SPI_TypeDef* SPI, void* Data, unsigned long Length);    //Length: 16 bit
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
/* Read fast
+) NOTE: pause spi bus (by interrupt, usb,...) maybe get wrong rx data
*/
void SPI_Reads(volatile SPI_TypeDef* SPI, void* Contain, unsigned long Length);  //Length: 16 bit
//-----------------------------------------------------------       LEVEL 1      -------------------------------------------------------------------------------
void SPI_WriteReads(volatile SPI_TypeDef* SPI, void* Contain, unsigned long Length);     //Length: 16 bit

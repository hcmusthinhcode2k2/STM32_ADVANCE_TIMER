...
//==================================================                 Using Functions            ================================================================
//==============================================================================================================================================================
void STK_Init(unsigned long Load);               //IRQ trigger = Load * 1000 / FSYS (us), FSYS KHz
//==============================================================================================================================================================
//============================================================        System timer         =====================================================================
//==============================================================================================================================================================
...
typedef struct
{
  BUNION(CTRL, unsigned long,                    //              0x0000'0000     SysTick control and status register
    ...
    _reserved1                          , 15);
  unsigned long LOAD;                            //              0x0000'0000     SysTick reload value register
  unsigned long VALUE;                           //              0x0000'0000     SysTick current value register
  unsigned long const CALIB;                     //              0x0000'0000     SysTick calibration value register
} STK_TypeDef;
...
typedef struct
{
  BUNION(CPUID, unsigned long const,             //              0x411F'C231     CPUID base register (STM32F2 and STM32L series: 0x412F'C230)
    REVISION                            , 4,    //0:3              
    PART_NO                             , 12,   //4:15          0xC23
    CONST                               , 4,    //16:19         0xF
    VARIANT                             , 4,    //20:23              
    IMPLEMENTER                         , 8);   //24:31         0x41
  BUNION(ICSR, unsigned long,                    //              0x0000'0000     Interrupt control and state register       
  ...
  BUNION(VTOR, unsigned long,                    //              0x0000'0000     Vector table offset register
    _reserved                           , 9,
    TABLEOFF                            , 21,   //9:29          0               Vector table base offset field
    _reserved1                          , 2);
  BUNION(AIRCR, unsigned long,                   //              0xFA05'0000     Application interrupt and reset control register               
  ...
  BUNION(SCR, unsigned long,                     //              0x0000'0000     System control register
    _reserved                           , 1,
    ...
    _reserved2                          , 27);
  BUNION(CCR, unsigned long,                     //              0x0000'0000     Configuration and control register ((STM32F2 and STM32L series: 0x0000'0200)
  ...
  BUNION(SHCRS, unsigned long,                   //              0x0000'0000     System handler control and state register
    MEM_FAULT_ACT                       , 1,    //0             0
    BUS_FAULT_ACT                       , 1,    //1             0
    _reserved                           , 1,
    USG_FAULT_ACT                       , 1,    //3             0
  ...
  BUNION(HFSR, unsigned long,                    //              0x0000'0000     Hard fault status register
    _reserved                           , 1,
    VECT_TBL                            , 1,    //1             0               Vector table hard fault
    _reserved1                          , 28,
    FORCED                              , 1,    //30            0               Forced hard fault
    DEBUG_VT                            , 1);   //31            0               Reserved for Debug use
  unsigned long MMFAR;                           //              0x----'----     Memory management fault address register
  unsigned long BFAR;                            //              0x----'----     Bus fault address register
} SCB_TypeDef;
#define SCB                                     NVIC.SCB_MASK
__root __no_init volatile
BUNION(SCB_ACTLR, unsigned long,                 //              0x0000'0000     Auxiliary control register
...
typedef struct
{
  union
  {
    RSTRUCT(REGS, unsigned long, r0, r1, r2);
    NVIC_BSTRUCT(1);
  } ISER;                                       //                              Interrupt set-enable registers
  unsigned long _reserved[29];
  union
  {
    RSTRUCT(REGS, unsigned long, r0, r1, r2);
    NVIC_BSTRUCT(1);
  } ICER;                                       //                              Interrupt clear-enable registers
  unsigned long _reserved1[29];
  union
  {
    RSTRUCT(REGS, unsigned long, r0, r1, r2);
    NVIC_BSTRUCT(1);
  } ISPR;                                       //                              Interrupt set-pending registers
  unsigned long _reserved2[29];
  union
  {
    RSTRUCT(REGS, unsigned long, r0, r1, r2);
    NVIC_BSTRUCT(1);
  } ICPR;                                       //                              Interrupt clear-pending registers
  unsigned long _reserved3[29];
  union
  {
    RSTRUCT(REGS, unsigned long const, r0, r1, r2);
    NVIC_BSTRUCT(1);
  } IABR;                                       //                              Interrupt active bit registers
  unsigned long _reserved4[61];
  union
  {
    xRSTRUCT(REGS, unsigned long, r, 0, 20,);
    NVIC_BSTRUCT(8);
  } IPR;  
  unsigned long _reserved5[555];
  SCB_TypeDef SCB_MASK;                         //0xE000ED00
  unsigned long _reserved6[114];
  unsigned long STIR;                            //              0x0000'0000     Software trigger interrupt register
} NVIC_TypeDef;
...
typedef struct
{
  BUNION(TYPER, unsigned long const,             //              0x0000'0800     MPU type register
    ...
    _reserved1                          , 8);
  BUNION(CR, unsigned long,                      //              0x0000'0000     MPU control register
    ...
    _reserved                           , 29);
  unsigned long REGION;                          //              0x0000'0000     MPU region number register
  STRUCT(A[4],
    BUNION(RBAR, unsigned long,                  //              0x0000'0000     MPU region base address register
      REGION                            , 4,    //0:3           0               MPU region field
      VALID                             , 1,    //4             0               MPU region number valid
      ADDR                              , 27);  //5:31          0               Region base address field
    BUNION(RASR, unsigned long,                  //              0x0000'0000     MPU region attribute and size register
